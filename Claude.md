...
**Quand les deux côtés ont pris LA MÊME décision, garder celui qui l'a mise à
l'endroit partagé.** Fusion nº 46 : la branche et main ont conclu, chacun de
son côté et sur la même mesure, que la fenêtre du logiciel devait être rendue
ENTIÈRE sur téléphone plutôt que rognée. La branche aiguillait en JS dans
`UseCasesBento` ; main a posé la règle DANS `OraAppScene` (`fit`, seuil 768 px),
où elle vaut pour tous les appelants. Garder les deux, ce serait deux
mécanismes pour un seul effet, dont un mort — et le mort finit par mentir. On
garde celui de main, et on supprime le nôtre, `useIsPhone` compris.

**Une règle générale venue de main peut casser un cas que la branche avait
réglé finement. Suivre la règle et réparer le cadre — ne pas se battre contre
la règle.** Même fusion. `OraAppScene` refuse désormais `cropScale` sous
768 px ; or le cadre du panneau « Contrôles et suivi » est une boîte FIXE de
1180 × 720 taillée pour être tranchée. La scène s'y ajustait donc à l'échelle 1
et la demi-colonne n'en montrait plus que les 181 px de gauche à taille réelle
— la barre latérale et rien d'autre, précisément l'accident que la nouvelle
règle veut supprimer. La tentation était d'exempter ce cadre dans
`OraAppScene` : c'eût été re-litiger la décision la plus récente de main. Le
cadre suit la règle — sur téléphone, une boîte au RAPPORT de la scène — et il
est CENTRÉ sur la cellule, que la grille étire à la hauteur de la colonne de
texte (542 px mesurés à 390 px, pour 110 px de scène : calée en haut, elle
laissait 432 px de gris vide).
⚠ Le compromis est assumé et il se voit : complète mais petite. Si la fenêtre
paraît trop petite dans la demi-colonne, la sortie honnête est d'EMPILER cette
rangée-là sur téléphone, pas de la re-rogner.

**Quand main refond une section entière, la branche ne défend pas ses tailles.**
Fusion nº 46 encore : main a rebâti la FAQ le 02/09 sur la grammaire large de
« Contrôle total » (conteneur max-w-7xl, très grand titre en deux blocs, neuf
questions en deux colonnes explicites). Cette branche n'y avait touché que les
CORPS de l'ancienne structure, laquelle n'existe plus — il n'y a rien à
composer, on prend la version de main entière. Une seule retouche, et elle sert
l'intention de main : la gouttière du téléphone passe à `px-5` pour que la FAQ
et « Contrôle total » partagent la même, main ayant demandé que les deux
sections se répondent.
