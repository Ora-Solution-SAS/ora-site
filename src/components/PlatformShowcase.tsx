...
function Shell({
  ...
  return (
    <motion.div
      ...
      className="relative overflow-hidden rounded-[18px] p-4 md:rounded-[26px] md:p-10 grid grid-cols-[minmax(0,1fr)_minmax(0,0.9fr)] items-center gap-4 lg:gap-14"
    >
      ...
      <div className="relative">
        ...
        <div className="flex items-center gap-3.5">
          <span className="grid h-8 w-8 place-items-center rounded-[10px] bg-white shadow-[0_2px_8px_-2px_rgba(10,37,64,0.18)] md:h-12 md:w-12 md:rounded-[14px] dark:bg-white/10">
            <img src="/logos/icon-color.png" alt="" aria-hidden className="h-6 w-auto select-none" draggable={false} />
          </span>
          <span className="font-instrument font-normal text-[1rem] md:text-[1.75rem] tracking-[-0.02em] text-[#111827] dark:text-white">
            {name}
          </span>
        </div>

        <h3 className="mt-3 max-w-[18ch] font-instrument font-normal text-[1.05rem] md:mt-6 md:text-[2rem] leading-[1.12] tracking-[-0.025em] text-[#111827] dark:text-white">
          {title}
        </h3>
        ...
        <button
          type="button"
          onClick={onCta}
          className="group mt-4 inline-flex items-center gap-2 rounded-[7px] bg-[#3b82f6] px-3.5 py-2 font-inter font-semibold text-[12px] text-white transition-colors duration-150 hover:bg-[#2563eb] md:mt-7 md:gap-2.5 md:px-5 md:py-3 md:text-[14.5px]"
        >
          {cta}
        ...
        {note && (
          <p className="mt-2.5 max-w-[34ch] font-inter text-[10.5px] leading-snug text-[#5b6577] md:mt-4 md:text-[13.5px] md:leading-relaxed dark:text-gray-400">
            {note}
          </p>
        )}
      </div>
...
export default function PlatformShowcase({ onNavigate }: PlatformShowcaseProps) {
  ...
  return (
    <section id="plateforme" data-nav-shy className="relative px-5 md:px-12 py-10 md:py-28 bg-white dark:bg-black">
